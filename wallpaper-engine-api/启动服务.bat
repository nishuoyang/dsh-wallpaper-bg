...
(
  ...
  echo # Scene rendering (full scene frames + baked animation video) is OFF by
  echo # default: it renders scene.pkg from your subscribed wallpapers and caches
  echo # the result to ~\.dsh-wallpaper-bg. Set WE_SCENE_RENDER=1 to enable it.
  echo WE_INSTALL_PATH=!INSTALL!
  echo WE_WORKSHOP_PATH=!USERWS!
  echo WE_SCENE_RENDER=0
) > "%~dp0we-api.config"
echo.
echo Saved to we-api.config.
echo Scene rendering is OFF by default (WE_SCENE_RENDER=0).
echo Need full scene frames / baked animation video? Edit we-api.config and
echo set WE_SCENE_RENDER=1, then restart the service.
