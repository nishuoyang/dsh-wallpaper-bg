...
export class Canvas {
  ...
  // blendMode > 0 → 与 blitScaled 相同的官方 ApplyBlending(mode, 画布色A, 源色B, 源alpha)
  // (lwe CImage: colorBlendMode 是 passthrough blend pass, 与是否旋转无关;
  //  旧实现旋转分支忽略 blendMode → 屏幕/叠加类旋转层被当不透明绘制, 如镜头光晕
  //  "彩虹弧+黑底" 纹理变成黑色菱形)
  blitRotated(img, cx, cy, dw, dh, angle, alpha = 1, blendMode = 0) {
