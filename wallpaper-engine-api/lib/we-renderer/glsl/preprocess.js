...
// preprocess 惰性取自 parser-loader（缺包时降级，不阻断渲染器加载）
import { getGlslPreprocess } from './parser-loader.js';
...
export function preprocessShader(source, { defines = {}, resolveInclude = null } = {}) {
  const preprocess = getGlslPreprocess();
  if (typeof preprocess !== 'function') throw new Error('GLSL 解析器未安装 (跳过该效果)');
  let src = source;
