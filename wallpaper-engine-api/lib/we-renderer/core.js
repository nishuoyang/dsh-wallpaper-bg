...
export class SceneRenderer {
  ...
  // ── 静态帧时刻选择 ────────────────────────────────────────────────
  // 壁纸当背景用时只取一帧。若这一帧正好落在「眨眼闭眼」相位, 角色看起来就像
  // 没有眼睛 (用户反馈的「场景壁纸角色眼睛没有或位置不对」即含此因)。
  // 做法: 从动画数据里找出「眼睑类骨骼」(scaleY 在动画中大幅变化, 典型眨眼),
  // 若默认时刻开度低, 就在候选时刻里挑开度最高、且对象可见度 (alpha 均值)
  // 不下降的时刻。完全基于已解析的动画数据, 不需要额外渲染。
  _collectPuppetEyes() {
    const out = [];
    if (!this._mdlCache) this._mdlCache = new Map();
    for (const o of this.objects) {
      if (!o.image) continue;
      const m = this.readJsonAny(o.image);
      if (!m || !m.puppet) continue;
      let mesh = this._mdlCache.get(m.puppet);
      if (!mesh) {
        mesh = this._parseMdl(this.pkg.read(m.puppet));
        if (mesh) this._mdlCache.set(m.puppet, mesh);
      }
      if (!mesh || !mesh.animations || !mesh.animations.length) continue;
      // 眨眼层一般是第一个可见的非 additive 层; 否则回退动画 0
      let animIdx = 0;
      let rate = 1;
      const layers = (o.animationlayers || []).filter((l) => l && l.visible === true);
      const layer = layers.find((l) => !l.additive) || layers[0];
      if (layer) {
        let idx = mesh.animations.findIndex((a) => a.name && layer.name && a.name === layer.name);
        if (idx < 0 && layer.name) {
          const mm = String(layer.name).match(/(\d+)/);
          if (mm) {
        ...
        if (idx < 0) {
          const li = o.animationlayers.indexOf(layer);
          if (li >= 0 && li < mesh.animations.length) idx = li;
        }
        if (idx >= 0) animIdx = idx;
        if (typeof layer.rate === 'number' && layer.rate > 0) rate = layer.rate;
      }
      const anim = mesh.animations[animIdx];
      if (!anim || !anim.segs) continue;
      const dv = new DataView(mesh.raw.buffer, mesh.raw.byteOffset, mesh.raw.byteLength);
      const total = Math.max(1, (anim.frameCount || 1) + 1);
      const step = Math.max(1, Math.floor(total / 32));
      const blinkBones = [];
      for (let b = 0; b < anim.segs.length; b++) {
        const seg = anim.segs[b];
        let min = Infinity, max = -Infinity;
        for (let f = 0; f < total; f += step) {
          const off = seg + f * 36;
          if (off + 36 > mesh.raw.length) break;
          const sy = dv.getFloat32(off + 28, true);
          if (isFinite(sy)) { if (sy < min) min = sy; if (sy > max) max = sy; }
        }
        if (max - min > 0.35) blinkBones.push({ b, min, max });
      }
      if (blinkBones.length) out.push({ anim, rate, blinkBones, dv, raw: mesh.raw });
    }
    return out;
  }

  /** 眼睑开度 0..1 (1 = 完全睁开); 无眨眼类骨骼时恒为 1 */
  _eyeOpenness(t) {
    if (!this._puppetEyes) this._puppetEyes = this._collectPuppetEyes();
    const eyes = this._puppetEyes;
    ...
    for (const e of eyes) {
      ...
      const total = Math.max(1, (anim.frameCount || 1) + 1);
      const frame = Math.floor(t * fps * e.rate) % total;
      for (const bone of e.blinkBones) {
        const off = anim.segs[bone.b] + frame * 36;
        if (!(off + 36 <= e.raw.length)) continue;
        const sy = e.dv.getFloat32(off + 28, true);
        if (!isFinite(sy)) continue;
        const v = (sy - bone.min) / ((bone.max - bone.min) || 1);
  ...
  /** 属性动画在该时刻的变化速率 (入场/淡入淡出动画 = 高, 稳定后 = 低) */
  _propMotionAt(t, dt = 0.15) {
    const sample = (tt) => {
      this._resolveAnimations(tt);
      const out = [];
      for (const o of this.objects) {
        if (o._renderType === 'particle') continue;
        const a = Number(getVal(o, 'alpha', 1));
        const br = Number(getVal(o, 'brightness', 1));
        out.push(isFinite(a) ? a : 0, isFinite(br) ? br : 0);
        const s = parseVec3(getVal(o, 'scale'), [1, 1, 1]);
        out.push(s[0], s[1]);
        const p = parseVec3(getVal(o, 'origin'), [0, 0, 0]);
        out.push(p[0] / 1000, p[1] / 1000);
      }
      return out;
    };
    ...
    try {
      ...
      this._resolveAnimations(prev);
    }
  }

  /**
   * 选静态帧时刻: 默认时刻眼睛睁着就照用; 若落在闭眼相位, 在候选时刻里换一个
   * 「入场动画已稳定 + 眼睑开度最大」的时刻。
   */
  pickStillTime(base) {
    const baseT = Number.isFinite(base) ? base : 2.5;
    try {
      if (this._eyeOpenness(baseT) >= 0.85) return baseT;
      const cands = [0, 0.4, 0.8, 1.2, 1.6, 2, 3, 3.4, 3.8, 4.2, 4.6, 5, 6, 7, 8, 10, 12];
      const open = cands.map((t) => ({ t, o: this._eyeOpenness(t) })).filter((c) => c.o >= 0.8);
      if (!open.length) return baseT;
      const scored = open.map((c) => ({ t: c.t, o: c.o, m: this._propMotionAt(c.t) }));
      const minM = Math.min(...scored.map((s) => s.m));
      // 入场动画未稳定的时刻 (淡入/淡出中) 先排除, 再比开度, 最后离默认时刻最近
      const settled = scored.filter((s) => s.m <= minM * 1.5 + 0.02);
      settled.sort((a, b) => (b.o - a.o) || (Math.abs(a.t - baseT) - Math.abs(b.t - baseT)));
      return settled[0].t;
    } catch {
      return baseT;
    }
