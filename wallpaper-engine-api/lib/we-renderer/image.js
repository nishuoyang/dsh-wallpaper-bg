...
,
    renderImage(o, t) {
        ...
        if (img && tr.angle !== 0) {
          ...
          // colorBlendMode 必须同时生效 (旧实现旋转分支丢失混合模式 → 旋转的
          // 屏幕/叠加层被画成不透明矩形)
          const rotated = img.rotated || img;
          const cbmRot = getVal(o, 'colorBlendMode', 0);
          this.canvas.blitRotated(rotated, dx + dw / 2, dy + dh / 2, dw, dh, tr.angle, alpha * brightness, cbmRot > 0 ? cbmRot : 0);
          return;
        }
...
,
    _renderSolidLayer(o, model, tr, t) {
        ...
        if (img && tr.angle !== 0) {
          ...
          // 同样保留 colorBlendMode (旋转 + 混合模式)
          const cbmRot = getVal(o, 'colorBlendMode', 0);
          this.canvas.blitRotated(img, dx + dw / 2, dy + dh / 2, dw, dh, tr.angle, alpha * brightness, cbmRot > 0 ? cbmRot : 0);
        } else if (img) {
