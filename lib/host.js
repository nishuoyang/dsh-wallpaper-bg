...
export default {
  ...
  // 预设行的插件要消费宿主服务，正确姿势是硬注入（dsh-tool-bash 等标准行
  // 同样硬注入 shell/systemPrompt 等宿主服务）；standing scope 下的 ctx.get
  // 看不到宿主提供的服务（实测 ctx.get('webServer') 返回空导致路由静默缺失）。
  // 'config' 是 cordis 内置注入，读取行配置必须声明。
  inject: ['config', 'webServer'],
  apply(ctx) {
