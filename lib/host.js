/**
 * dsh-wallpaper-bg v0.3.12 — 宿主半（Node 侧）
 * ===========================================
 ...
 *                                              本地资源流式代理（壁纸图片/视频文件）
 *   GET /dsh-wallpaper-bg/scene-frame?id=...&w=&t=...
 *                                              场景壁纸完整帧同源代理（转发 WE API 服务）
 *
...
function normalizeItem(item) {
  ...
  return {
    ...
    previewUrl,
    // 场景壁纸：WE API 服务是否可渲染完整场景帧（scene.pkg → 对象树/骨骼/粒子/shader）
    hasFrame: item.hasFrame === true,
...
export default {
  ...
  // 预设行插件的正确取法（与标准行 tool-pwsh / tool-web 一致）：
  //  - 宿主服务硬注入：inject: ['webServer']（standing mount 下 ctx.get 取不到）；
  ...
  apply(ctx, config) {
    ...
    async function fetchWe(base, endpoint, refresh) {
      const cleanBase = String(base).replace(/\/+$/, '')
      const candidates = ENDPOINTS[endpoint] || ['/']
      let lastError = null
      for (const ep of candidates) {
        try {
          const url = cleanBase + ep + (refresh ? '?refresh=1' : '')
    ...
    async function weApi(query) {
      ...
      const raw = await fetchWe(base, endpoint, refresh)
      let payload
      if (endpoint === 'ping') {
        // ping 也透传场景渲染能力位：1/true = 服务端场景渲染可用；
        // 0/false = 服务端默认关闭（WE 服务 0.3.1+ 默认关，场景壁纸回退预览图）；
        // 缺省 = 旧版服务，客户端仍尝试并自动回退。
        payload = {
          reachable: true,
          sceneRender:
            raw && raw.sceneRender !== undefined
              ? (raw.sceneRender === 1 || raw.sceneRender === true)
              : undefined,
        }
        ...
        payload = {
          ...
          // WE API 服务是否支持场景完整帧渲染（/scene-frame）。1/true = 可用；
          // 0/false = 服务端默认关闭（不建 ~/.dsh-wallpaper-bg 缓存目录，场景壁纸
          // 回退 preview.gif / preview.jpg）；缺省 = 旧版服务，客户端会尝试并自动回退。
          sceneRender:
            raw && raw.sceneRender !== undefined
              ? (raw.sceneRender === 1 || raw.sceneRender === true)
              : undefined,
      ...
      handler: async (req, res) => {
        try {
          ...
          if (p === '/dsh-wallpaper-bg/health') {
            return sendJson(res, 200, { ok: true, plugin: 'dsh-wallpaper-bg', version: '0.3.12' })
          }

          if (p === '/dsh-wallpaper-bg/scene-frame') {
            // 场景壁纸完整帧（同源代理）：转发到 WE API 服务的 /scene-frame/<id>。
            // 客户端默认直连服务（服务已发 CORS 头）；此路由留给「客户端只想走同源」的场景。
            // 服务端 worker 渲染 + 磁盘缓存，失败（422）时客户端自动回退预览图。
            const id = url.searchParams.get('id') || ''
            if (!/^\d{1,20}$/.test(id)) return sendJson(res, 400, { ok: false, error: '非法壁纸 ID' })
            const base = String(url.searchParams.get('base') || defaultBase).replace(/\/+$/, '')
            ...
            try {
              ...
              res.writeHead(upstream.status, {
                ...
                'X-Scene-Mode': upstream.headers.get('x-scene-mode') || '',
                'X-Scene-Cached': upstream.headers.get('x-scene-cached') || '',
              })
              return res.end(buf)
            } catch (error) {
