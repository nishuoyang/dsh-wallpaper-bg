/**
 * dsh-wallpaper-bg v0.3.11 — 宿主半（Node 侧）
 * ===========================================
    ...
    async function weApi(query) {
      ...
      if (endpoint === 'ping') {
        ...
        payload = {
          ...
          // WE API 服务是否支持场景完整帧渲染（/scene-frame）。旧版服务没有该字段
          // → 客户端仍会尝试，失败自动回退 preview.gif / preview.jpg。
          sceneRender: raw && (raw.sceneRender === 1 || raw.sceneRender === true) ? true : undefined,
        }
      }
      ...
      handler: async (req, res) => {
        try {
          ...
          if (p === '/dsh-wallpaper-bg/health') {
            return sendJson(res, 200, { ok: true, plugin: 'dsh-wallpaper-bg', version: '0.3.11' })
          }
