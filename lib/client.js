/**
 * dsh-wallpaper-bg v0.3.12 — 浏览器半（单文件 client bundle）
 * ==========================================================
...
window.__ModuleLoader__.load({
  ...
  factory(require) {
    ...
    var DEFAULTS = {
      ...
      sceneAnim: true,       // 场景壁纸有烘焙动画时用 <video> 播放（静态帧 → 会动的循环视频）
      sceneAnimAutoBake: false, // 选中未烘焙的场景壁纸时自动开始烘焙（默认关：烘焙很吃 CPU）
      sceneAnimWidth: 1920,  // 烘焙分辨率
      sceneAnimFps: 24,      // 烘焙帧率
      sceneAnimDuration: 4,  // 循环时长（秒）
    }
    var BUILTIN = [
    ...
    function apply(ctx) {
      ...
      var shimHinted = false   // 旧版 WE API 服务的提示只打一次
      ...
      function preloadWallpaper(source, id, ready) {
        ...
        if (source === 'builtin') {
          ...
          if (item.kind === 'video') {
            ...
            // 场景帧是服务端渲染的静态图：提前解码，切换时直接叠化（否则要等渲染 + 下载）
            var fu = sceneFrameUrlOf(item)
            p = fu
              ? loadImage(fu).then(function () { return { url: fu, objectUrl: false } }, function () { return null })
              : Promise.resolve(null)
          } else {
            p = Promise.resolve(null)   // 网页由 iframe 自己加载
          }
        }
      ...
      /** 与 applyImage 同语义，但加载失败时调用 onFail（场景帧 → 预览图回退用）。
       *  抽成独立函数而不是给 applyImage 加参数：applyImage 的调用点很多，
       *  保持其签名不变可避免误传参数导致行为漂移。 */
      function applyImageOrFallback(url, onFail) {
      ...
      /** 场景壁纸完整帧地址（WE API 服务的 /scene-frame：worker 渲染 scene.pkg 全场景）。
       *  直连服务（服务已发 Access-Control-Allow-Origin: *），客户端不必重启 DSH 就能用；
       *  w 限制传输体积，t 给渲染器一个稳定时刻——t=0 时很多壁纸还停在黑幕/淡入的第一帧，
       *  2.5s 才是"正片"画面（与上游 dsh-plugin-wallpaper-engine 同取值）。
       *  weSceneRender === false 表示服务端场景渲染已关闭（默认关，不建缓存目录）：
       *  直接返回 '' 走工坊预览图，不发起注定 403 的请求。 */
      function sceneFrameUrlOf(item) {
        if (!item) return ''
        // 服务端默认关闭场景渲染（WE API 服务 0.3.1+，WE_SCENE_RENDER=1 才开启）：
        // 场景壁纸回退 preview.gif / preview.jpg
        if (weSceneRender === false) return ''
        // hasFrame=false 表示服务端确认没有 scene.pkg（别白请求）；未定义说明宿主半还是旧版
        // 没有透传该字段——仍然试一次，失败走预览图兜底。
        if (item.hasFrame === false) return ''
        if (item.kind && item.kind !== 'scene') return ''
        var base = String(state.weBase || DEFAULTS.weBase).replace(/\/+$/, '')
        return base + '/scene-frame/' + encodeURIComponent(String(item.id)) + '?w=2560&t=2.5'
      }

      /** 场景预览图地址：优先工坊预览图（preview.gif 会动），退回缩略图 */
      function scenePreviewUrlOf(item) {
        if (!item) return ''
        return assetUrlOf(item.previewFile || item.thumbnail)
      }

      // ---------- 场景壁纸动画烘焙（把场景渲染成可循环 MP4，<video> 原生播放） ----------
      // 服务端 /scene-anim/<id>：异步任务（渲染 N 帧 → 检测循环点 → ffmpeg 编码），
      // 完成后 /scene-anim/<id>/video.mp4 就是一段无缝循环视频。烘焙很吃 CPU（普通
      // 场景 1080p 约 1–2 分钟，重效果场景 5–25 分钟），因此默认只播已烘焙好的，
      // 需要时由用户点按钮（或打开「选中时自动烘焙」）触发。
      var sceneAnimCache = {}   // id → { url, meta }：已知烘焙完成的视频
      var sceneAnimJobs = {}    // id → { state, percent, done, total, error }：进行中的任务
      var sceneAnimPollIv = null

      function sceneAnimBase() {
        return String(state.weBase || DEFAULTS.weBase).replace(/\/+$/, '')
      }
      function sceneAnimQuery() {
        return 'w=' + (Number(state.sceneAnimWidth) || DEFAULTS.sceneAnimWidth) +
          '&fps=' + (Number(state.sceneAnimFps) || DEFAULTS.sceneAnimFps) +
          '&dur=' + (Number(state.sceneAnimDuration) || DEFAULTS.sceneAnimDuration)
      }
      function sceneAnimStatusUrl(item, bake, cancel) {
        return sceneAnimBase() + '/scene-anim/' + encodeURIComponent(String(item.id)) + '?' + sceneAnimQuery() +
          (bake ? '&bake=1' : '') + (cancel ? '&cancel=1' : '')
      }
      function sceneAnimVideoUrl(item, version) {
        return sceneAnimBase() + '/scene-anim/' + encodeURIComponent(String(item.id)) + '/video.mp4?' + sceneAnimQuery() +
          (version ? '&v=' + encodeURIComponent(version) : '')
      }

      /** 当前实际显示的 WE 壁纸 id（队列优先，与 applyNow 的分发逻辑一致） */
      function activeWeItemId() {
        var wq = state.weQueue || []
        if (state.weQueueEnabled && wq.length) {
          var i = Math.min(Math.max(0, Number(state.weQueueIndex) || 0), wq.length - 1)
          return String(wq[i])
        }
        return String(state.wallpaperId || '')
      }

      function ensureSceneAnimPoll() {
        if (sceneAnimPollIv) return
        sceneAnimPollIv = window.setInterval(function () {
          var ids = Object.keys(sceneAnimJobs).filter(function (id) {
            var j = sceneAnimJobs[id]
            return j && (j.state === 'running' || j.state === 'queued')
          })
          if (!ids.length) {
            window.clearInterval(sceneAnimPollIv)
            sceneAnimPollIv = null
            return
          }
          ids.forEach(function (id) {
            fetch(sceneAnimBase() + '/scene-anim/' + encodeURIComponent(id) + '?' + sceneAnimQuery())
              .then(function (r) { return r.json() })
              .then(function (st) {
                if (!st || st.ok === false) return
                sceneAnimJobs[id] = {
                  state: st.state, percent: st.percent || 0, done: st.done || 0,
                  total: st.total || 0, error: st.error || null,
                }
                if (st.state === 'done') {
                  var meta = st.meta || {}
                  sceneAnimCache[id] = { url: sceneAnimVideoUrl({ id: id }, meta.createdAt || ''), meta: meta }
                  delete sceneAnimJobs[id]
                  // 正在显示这张 → 立刻换成动画（静态帧 → 视频的叠化由 crossfade 负责）
                  if (activeWeItemId() === id && state.sceneAnim !== false) applyVideo(sceneAnimCache[id].url)
                }
                emit()
              }, function () { /* 网络抖动：下一轮再试 */ })
          })
      ...
      /** 查询烘焙状态（bake=true 时入队）；结果写入 sceneAnimJobs / sceneAnimCache */
      function sceneAnimQueryStatus(item, bake) {
        if (!item || !item.id) return Promise.resolve(null)
        var id = String(item.id)
        return fetch(sceneAnimStatusUrl(item, !!bake)).then(function (r) { return r.json() }).then(function (st) {
          if (!st || st.ok === false) return null
          if (st.state === 'done') {
            var meta = st.meta || {}
            sceneAnimCache[id] = { url: sceneAnimVideoUrl(item, meta.createdAt || ''), meta: meta }
            delete sceneAnimJobs[id]
          } else {
            sceneAnimJobs[id] = {
              state: st.state, percent: st.percent || 0, done: st.done || 0,
            ...
            if (st.state === 'running' || st.state === 'queued') ensureSceneAnimPoll()
          }
          emit()
          return st
        }, function () { return null })
      ...
      /** 用户点击「烘焙动画」：入队并开始轮询 */
      function startSceneAnim(item) {
        if (!item || !item.id) return
        var id = String(item.id)
        sceneAnimJobs[id] = { state: 'queued', percent: 0, done: 0, total: 0, error: null }
        emit()
        ensureSceneAnimPoll()
        sceneAnimQueryStatus(item, true)
      }

      function cancelSceneAnim(item) {
        if (!item || !item.id) return
        var id = String(item.id)
        fetch(sceneAnimStatusUrl(item, false, true)).then(function (r) { return r.json() }).then(function () {
          delete sceneAnimJobs[id]
          emit()
        }, function () { /* 忽略 */ })
      }

      function applyScene(item, preloadedFrame) {
        if (!item) { clearActive(); return }
        var id = String(item.id)
        var preview = scenePreviewUrlOf(item)
        // 已烘焙 + 开关打开 → 直接播循环视频（最生动）
        var baked = state.sceneAnim !== false ? sceneAnimCache[id] : null
        if (baked && baked.url) { applyVideo(baked.url); return }
        var frame = preloadedFrame || sceneFrameUrlOf(item)
        var showStill = function () {
          if (frame) {
            return applyImageOrFallback(frame, function () {
              if (preview) applyImage(preview)
              else clearActive()
            })
          }
          // 旧版 WE API 服务（无场景渲染）或该壁纸没有 scene.pkg：退回工坊预览图
          return preview ? applyImage(preview) : clearActive()
        }
        // 先出静态帧（不让人等），同时问一下烘焙状态：已烘焙 → 换视频；在烘焙 → 显示进度
        // 服务端场景渲染默认关闭（weSceneRender === false）时跳过：烘焙接口同样 403
        if (state.sceneAnim !== false && weSceneRender !== false) {
          sceneAnimQueryStatus(item, false).then(function (st) {
            if (!st) return
            if (st.state === 'done') {
              if (activeWeItemId() === id) applyVideo(sceneAnimCache[id].url)
            } else if (st.state === 'idle' && state.sceneAnimAutoBake) {
              startSceneAnim(item)
            }
          })
        }
      ...
      /** iframe 类壁纸（场景 / 网页）：插入 DOM 后再设 src，等「已画出内容」的信号再叠化入场。
       *  两个要点：
       *  1. 未插入文档的 iframe 没有浏览上下文——设了 src 也不会加载、load 永不触发、
       *     contentDocument 为 null。旧实现先设 src、等 load 时才插入，于是只能靠 4 秒
       *     兜底定时器插入后才开始加载（网页壁纸因此要等 4 秒以上，且插入瞬间就叠化，
       *     第三方页面还没画出第一帧 → 黑屏）。现在先入 DOM 再设 src，加载零延迟；
       *     图层 opacity 仍为 0，所以提前入 DOM 不会提前露出内容。
       *  2. 网页壁纸的 WE 兼容垫片（由 WE API 的 /files 路由注入）在真正画出内容时
       *     postMessage 上报，此时立刻叠化最准；没有垫片时退化为 load + 500ms。 */
      function applyFrame(url, title, kind) {
        var seq = ++applySeq
        var origin = ''
        try { origin = new URL(url, location.href).origin } catch (e) { origin = '' }
        var iframe = document.createElement('iframe')
        iframe.setAttribute('scrolling', 'no')
        iframe.setAttribute('title', title)
        // 网页壁纸常自带 BGM / 音效，没有 autoplay 授权时 play() 被拒，部分壁纸会卡在初始化
        ...
        var detach = null   // 移除垫片消息监听（图层被清理时也要摘掉，否则快速切换会累积监听器）
        ...
        return new Promise(function (resolve) {
          ...
          onMessage = function (ev) {
            ...
            var d = ev.data
            if (!d || d.__wbg !== 1) return
            gotShim = true
            if (d.type === 'painted') finish()
            else if (d.type === 'error' && d.payload) {
              // 壁纸自身脚本报错（跨域页面只能靠垫片转发），留在控制台便于排查黑屏
              try { console.warn('[dsh-wallpaper-bg] 网页壁纸 ' + title + ' 报错：' + d.payload.message) } catch (e) { /* 忽略 */ }
            }
          }
          window.addEventListener('message', onMessage)
          // 先入 DOM（此时图层透明）再设 src：iframe 立刻开始加载
          bgEl.appendChild(layer)
          // 没有垫片（旧版 WE API 服务 / 第三方页面）时的兜底：load 后再留一点渲染时间
          iframe.addEventListener('load', function () { later(finish, 500) })
          iframe.setAttribute('src', url)
          later(finish, 8000)   // 总兜底：绝不让旧壁纸一直挂着
          // 一条 __wbg 消息都收不到 → WE API 服务多半还是旧版（没有属性垫片与 ../ 路径兜底），
          // 这正是网页壁纸黑屏 / 只剩角色像竖屏壁纸的常见原因，提示一次便于用户自查
          later(function () {
            if (gotShim || shimHinted || !/\/files\/\d+\//.test(url)) return
            shimHinted = true
            try {
              console.info('[dsh-wallpaper-bg] 未收到网页壁纸兼容垫片上报：WE API 服务可能是旧版本，'
                + '网页壁纸可能黑屏或缺少背景。升级后双击 wallpaper-engine-api/重启服务(管理员).bat 即可。')
            } catch (e) { /* 忽略 */ }
          }, 2500)
      ...
      // 服务端场景渲染能力位：null = 未知（旧版服务，仍尝试渲染帧）；false = 服务端
      // 默认关闭（WE_SCENE_RENDER=1 才开启，不建 ~/.dsh-wallpaper-bg，场景壁纸回退
      // 工坊预览图）；true = 可用。由 noteWeList 从 /dsh-wallpaper-bg/we 响应更新。
      var weSceneRender = null
      /** 吸收一次 WE 列表响应：缓存列表并记录服务端场景渲染能力位 */
      function noteWeList(res) {
        if (!res || !res.ok) return
        weCache.list = res.wallpapers || []
        if (res.sceneRender !== undefined) {
          weSceneRender = res.sceneRender === true || res.sceneRender === 1
        }
      }
      var applyToken = 0
      async function applyNow() {
        ...
        function applyWeItem(item) {
          ...
          if (item.kind === 'scene') {
            var preS = takePreload('we', item.id)
            applyScene(item, preS.url || '')
            return
          }
          if (item.kind === 'web') {
            applyWeb(item)
      ...
      function WallpaperPanel(props) {
        ...
        // 打开面板 / 切换壁纸时刷新一次「当前场景壁纸的烘焙状态」，让状态行不是旧值
        var animCurId = activeWeItemId()
        React.useEffect(function () {
          if (snap.source !== 'we' || !animCurId) return
          var cur = (weList || (weCache && weCache.list) || []).find(function (x) { return String(x.id) === animCurId })
          if (!cur || cur.kind !== 'scene') return
          sceneAnimQueryStatus(cur, false)
        }, [animCurId, snap.source, weList])
        ...
        return React.createElement('div', { className: 'wbg-panel' },
          ...
          // ---------- 场景壁纸动画（烘焙成循环视频） ----------
          (function () {
            // 服务端场景渲染默认关闭（WE_SCENE_RENDER=1 才开启）：/scene-anim 返回 403，
            // 这里不显示烘焙控件，只提示开启方式（避免「点了没反应」）。
            if (weSceneRender === false) {
              return React.createElement('div', { className: 'wbg-row', style: { flexWrap: 'wrap', gap: '8px' } },
                React.createElement('span', { className: 'wbg-hint', style: { flexBasis: '100%' } },
                  '场景渲染（完整场景帧 / 烘焙动画）默认关闭：服务端不会创建 ~/.dsh-wallpaper-bg 缓存目录。' +
                  '需要时在 WE API 服务的 we-api.config 里加一行 WE_SCENE_RENDER=1 并重启服务，这里才会出现烘焙开关。'),
              )
            }
            var curId = activeWeItemId()
            var cur = (weList || (weCache && weCache.list) || []).find(function (x) { return String(x.id) === curId })
            var isScene = !!(cur && cur.kind === 'scene')
            var job = sceneAnimJobs[curId]
            var baked = sceneAnimCache[curId]
            var statusText
            if (!isScene) statusText = '当前不是场景壁纸（场景类壁纸才会烘焙动画）'
            else if (job && job.state === 'running') statusText = '烘焙中 ' + job.percent + '%（' + job.done + '/' + job.total + ' 帧）'
            else if (job && job.state === 'queued') statusText = '排队中…'
            else if (job && job.state === 'error') statusText = '烘焙失败：' + (job.error || '未知错误')
            else if (job && job.state === 'static') statusText = '这张壁纸本身没有动画（所有帧几乎一致）'
            else if (baked) statusText = '已烘焙：' + (baked.meta && baked.meta.durationSec ? baked.meta.durationSec + ' 秒循环' : '循环视频') +
              (baked.meta && baked.meta.width ? ' · ' + baked.meta.width + '×' + baked.meta.height : '') +
              (baked.meta && baked.meta.fps ? ' · ' + baked.meta.fps + 'fps' : '')
            else statusText = '未烘焙——烘焙后粒子 / 水面 / 角色呼吸都会动'
            return React.createElement('div', { className: 'wbg-row', style: { flexWrap: 'wrap', gap: '8px' } },
              React.createElement('label', { className: 'wbg-switch', style: { whiteSpace: 'nowrap' } },
                React.createElement('input', {
                  type: 'checkbox',
                  checked: snap.sceneAnim !== false,
                  onChange: function (e) { setState({ sceneAnim: e.target.checked }) },
                }),
                '场景壁纸播放烘焙动画',
              ),
              React.createElement('label', { className: 'wbg-switch', style: { whiteSpace: 'nowrap' } },
                React.createElement('input', {
                  type: 'checkbox',
                  checked: !!snap.sceneAnimAutoBake,
                  onChange: function (e) { setState({ sceneAnimAutoBake: e.target.checked }) },
                }),
                '选中时自动烘焙',
              ),
              isScene && !(job && (job.state === 'running' || job.state === 'queued'))
                ? React.createElement('button', {
                    type: 'button', className: 'wbg-btn',
                    onClick: function () { startSceneAnim(cur) },
                  }, baked ? '重新烘焙' : '烘焙动画')
                : null,
              isScene && job && (job.state === 'running' || job.state === 'queued')
                ? React.createElement('button', {
                    type: 'button', className: 'wbg-btn',
                    onClick: function () { cancelSceneAnim(cur) },
                  }, '取消烘焙')
                : null,
              React.createElement('span', { className: 'wbg-hint', style: { flexBasis: '100%' } }, statusText),
            )
          })(),
          React.createElement('div', { className: 'wbg-hint' }, '烘焙把场景渲染成一段可循环的 MP4（后台进行，不影响使用）：普通壁纸约 1–2 分钟，效果很重的可能 5–25 分钟。循环点由服务端自动检测，播放时不会跳帧。鼠标视差与音频响应类效果无法烘焙（它们是实时交互的）。'),
